...
const default_file_path : String = "user://save_data.save"
...
var current_state_dictionary := {} :
	set(value):
		current_state_dictionary = value

var current_state_dictionary_persistent := {} 
...
func has_persistent(key_path : String) -> bool:
	if _is_hierarchical(key_path):
		var variable_head = _get_variable_name_head(key_path)
		var key_parent = _get_variable_at_path(_get_variable_name_body(key_path))
		if key_parent == null:
		...
		return current_state_dictionary_persistent.has(key_path)
...
func set_persistent_var(key_path : String, value):
	key_path = _sanitize_key_path(key_path)
	if _is_hierarchical(key_path):
		if value is Resource:
			value = _resource_to_dict(value)
	...
	if value is Resource: # Resource, has to be nested and translated.
		current_state_dictionary_persistent[key_path] = _resource_to_dict(value)
	else: # Simple value.
		current_state_dictionary_persistent[key_path] = value
...
func save() -> bool:
	...
	if current_state_dictionary.is_empty() and Engine.is_editor_hint():
		if !can_save_empty_in_editor_with_permission:
			return false
	if current_state_dictionary.is_empty():
		return false
	var file : FileAccess
	if use_encryption:
		file = FileAccess.open_encrypted_with_pass(SaveManager.get_current_path(), FileAccess.WRITE, encryption_key)
	else:
		file = FileAccess.open(SaveManager.get_current_path(), FileAccess.WRITE)
	...
	if !current_state_dictionary or !file:
		return false
...
func save_persistent() -> bool:
	...
	var file : FileAccess
	if use_encryption:
		file = FileAccess.open_encrypted_with_pass(SaveManager.get_persistent_path(), FileAccess.WRITE, encryption_key)
	else:
		file = FileAccess.open(SaveManager.get_persistent_path(), FileAccess.WRITE)
	
	# We need to call typecasting when saving too, so that Resources that
	# weren't manually added with set_var but are parts of an object that
	# were, can be broken down to Dictionaries.
	current_state_dictionary_persistent = _typecast_dictionary_keys(current_state_dictionary_persistent)
	if !current_state_dictionary_persistent or !file:
		return false
	file.store_string(JSON.stringify(current_state_dictionary_persistent, "\t"))
	can_save_empty_in_editor_with_permission = false
	saved.emit()
	return true
...
func get_var(key_path : String, default = null):
	...
	if var_at_path != null:
		...
		print(key_path, " returning default")
		return default

func get_persistent_var(key_path : String, default = null):
	return get_var_from_save(SaveManager.get_persistent_path(), key_path, default)


func get_var_from_save(save_path : String, key_path : String, default = null):
	key_path = _sanitize_key_path(key_path)
	var save_state_dict = _get_save_state_dict(save_path)
	if !save_state_dict:
		print("corrupted save file")
		return default
	if !save_state_dict.has(key_path):
		return default
	var var_at_path = save_state_dict[key_path]
	if var_at_path != null:
		return var_at_path
	else:
		print(key_path, " returning default")
...
func _load_persistent():
	var file : FileAccess
	if use_encryption:
		file = FileAccess.open_encrypted_with_pass(SaveManager.get_persistent_path(), FileAccess.READ, encryption_key)
	else:
		file = FileAccess.open(SaveManager.get_persistent_path(), FileAccess.READ)
	
	if file and !file.get_as_text().is_empty():
		current_state_dictionary_persistent = JSON.parse_string(file.get_as_text())
		if perform_typecast_on_dictionary_keys:
			current_state_dictionary_persistent = _typecast_dictionary_keys(current_state_dictionary_persistent)
	
	emit_signal("loaded")

func _get_save_state_dict(file_path : String):
	var file : FileAccess
	var state_dict
	if use_encryption:
		file = FileAccess.open_encrypted_with_pass(file_path, FileAccess.READ, encryption_key)
	...
	if file:
		state_dict = JSON.parse_string(file.get_as_text())
		if perform_typecast_on_dictionary_keys:
			state_dict = _typecast_dictionary_keys(current_state_dictionary)
